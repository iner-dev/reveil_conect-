...
void setup() {
  ...
  delay(1000); 
  noTone(BUZZER_PIN);
  next_event.start = 32503680000;
  next_event.end   = 32503680001;
...
void loop() {
    ...
    if(( ( (last_update+UPDATE_MIN_DELAY)<now && ((next_event.start-UPDATE_EVENT_LOCK_DELAY)>now)) ) || (!secret_mode && !digitalRead(UPDATE_PIN))){  // updating 
      update();
    }//*/
